...
define(['jquery', 'core/log', 'core/config'], function($, log, cfg) {
    ...
    var vmoodlepro = {
        ...
        init: function() {
            ...
            $('#id_s_local_vmoodle_licensekey').bind('change', this.check_product_key);
            $('#id_s_local_vmoodle_licensekey').trigger('change');
            log.debug('AMD Pro js initialized for local VMoodle');
        },
        ...
        check_product_key: function() {
            ...
            var calculated = vmoodlepro.checksum(payload);
            ...
            if (crc == calculated) {
                ...
                url += '&provider=' + $('#id_s_local_vmoodle_licenseprovider').val();

                $('#id_s_local_vmoodle_licensekey + img').remove();
                $('#id_s_local_vmoodle_licensekey').after(waiticon);

                $.get(url, function(data) {
                    if (data.match(/SET OK/)) {
                        $('#id_s_local_vmoodle_licensekey + img').remove();
                        $('#id_s_local_vmoodle_licensekey').after(validicon);
                    } else {
                        $('#id_s_local_vmoodle_licensekey + img').remove();
                        $('#id_s_local_vmoodle_licensekey').after(invalidicon);
                    }
                }, 'html');
            } else {
                $('#id_s_local_vmoodle_licensekey + img').remove();
                $('#id_s_local_vmoodle_licensekey').after(cautionicon);
            }
        },
