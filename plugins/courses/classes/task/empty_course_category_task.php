 ...
 * @license     http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class empty_course_category_task extends adhoc_task {
    ...
    public function execute() {
        ...
        if (!$coursecat = $DB->get_record('course_categories', array('idnumber' => $customdata->catidnumber))) {
            mtrace(get_string('errornocoursecategory', 'vmoodleadminset_courses'));
            return false;
        }

        $cat = \core_course_category::get($coursecat->id);
        $children = $cat->get_children();
        if (!empty($children)) {
            foreach ($children as $child) {
                $child->delete_full(true);
            }
        }
