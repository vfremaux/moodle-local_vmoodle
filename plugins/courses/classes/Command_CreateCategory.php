 ...
 */
class Command_CreateCategory extends Command {
    ...
    /**
     * Constructor.
     * @throws Command_CreateCategory_Exception
     */
    public function __construct() {
        global $vmcommandconstants;

        $name = vmoodle_get_string('cmdcreatecategory', 'vmoodleadminset_courses');
        $description = vmoodle_get_string('cmdcreatecategory_desc', 'vmoodleadminset_courses');

        // Create a category gven a name path from root.
        $parameters[] = new Command_Parameter(
            'path',
            'text',
            vmoodle_get_string('path', 'vmoodleadminset_courses'),
            null,
            null,
            array('size' => 100));
        ...
        $parameters[] = new Command_Parameter(
            'visible',
            'boolean',
            vmoodle_get_string('catvisible', 'vmoodleadminset_courses'),
            1,
            null);
    ...
    public function run($hosts) {
        ...
        if (!has_capability('local/vmoodle:execute', \context_system::instance())) {
            throw new Command_CreateCategory_Exception('insuffisantcapabilities');
        }
        ...
        $rpcclient->set_method('local/vmoodle/plugins/courses/rpclib.php/mnetadmin_rpc_create_category');
        $rpcclient->add_param($this->get_parameter('path')->get_value(), 'string');
        $rpcclient->add_param($this->get_parameter('idnumber')->get_value(), 'string');
        $rpcclient->add_param($this->get_parameter('visible')->get_value(), 'int');
