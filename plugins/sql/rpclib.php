...
function mnetadmin_rpc_get_fields($user, $table, $fields, $select) {
    ...
    $params = array($field[0] => $select[$field[0]],
                    isset($select[1]) ? $field[1] : '' => isset($select[1]) ? $select[1] : '',
                    isset($select[2]) ? $field[2] : '' => isset($select[2]) ? $select[2] : '');
    $record = $DB->get_record($table, $params, implode(',', $fields));
