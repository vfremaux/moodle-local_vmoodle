...
require_once($CFG->dirroot.'/local/vmoodle/classes/core_task_manager.php');
...
if (!$task) {
    throw new moodle_exception(get_string('cannotfindinfo', 'error', $taskname));
}

if (!\core\task\manager::is_runnable()) {
    $redirecturl = new \moodle_url('/admin/settings.php', ['section' => 'systempaths']);
    throw new moodle_exception(get_string('cannotfindthepathtothecli', 'tool_task'));
}

if (!get_config('tool_task', 'enablerunnow') || !$task->can_run()) {
    throw new moodle_exception(get_string('nopermissions', 'error', get_string('runnow', 'tool_task', $task->get_name())));
}
...
// CHANGE+.
\local\vmoodle\core_task_manager::run_from_cli($task);
// CHANGE-.
