...
class manager {
    ...
    public static function run_from_cli(\core\task\task_base $task):bool {
        ...
        if (!self::is_runnable()) {
            ...
            // PATCH+ : Vmoodle : mention hostname and call vmoodle version.
            // @ref local_vmoodle
            if (is_dir($CFG->dirroot.'/local/vmoodle')) {
                // If VMoodle present, prefer VMoodle version to bind host.
                $pathcomponents = [$CFG->dirroot, 'local/vmoodle', 'cli', 'schedule_task.php'];
                $scriptpath     = escapeshellarg(implode(DIRECTORY_SEPARATOR, $pathcomponents)).' --host="'.$CFG->wwwroot.'" ';
            }
            // PATCH-.
