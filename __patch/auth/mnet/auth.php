...
// PATCH+ : allow multijump and admin hands on.
/*
...
// PATCH-.
    ...
    function confirm_mnet_session($token, $remotepeer) {
        ...
        if ($mnetrequest->send($remotepeer) === true) {
            ...
            throw new \moodle_exception(get_string("rpcerror", 'mant', $message));
        }
        unset($mnetrequest);

        if (empty($remoteuser) or empty($remoteuser->username)) {
            throw new \moodle_exception(get_string('unknownerror', 'mnet'));
            exit;
        }
        ...
        // PATCH+ : Remove field name filter.
        if (empty($CFG->disablemnetimportfilter)) $remoteuser = mnet_strip_user($remoteuser, mnet_fields_to_import($remotepeer));
        // PATCH-.
        ...
        // PATCH+ : Resolve same IDNumber identity.

        // We may have a primary assignation that should be respected. @see bloc User_Mnet_Host
        $umhconfig = get_config('block_user_mnet_hosts');
        if (is_dir($CFG->dirroot.'/blocks/user_mnet_hosts') && !empty($umhconfig->singleaccountcheck)) {
            require_once($CFG->dirroot.'/blocks/user_mnet_hosts/xlib.php');
            $localuser = user_mnet_hosts_get_local_user($remoteuser, $remotehost);
        }
        // PATCH-.
        ...
        // PATCH+ : Add multimnet support.
        if (($localuser->auth == 'mnet') || ($localuser->auth == 'multimnet')) {
