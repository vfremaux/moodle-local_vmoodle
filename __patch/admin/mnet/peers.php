...
if (!empty($hostid)) {
    ...
    if ($hostid != $CFG->mnet_all_hosts_id) {
        // PATCH+ : Secures key comparison.
        $mnet_peer->currentkey = str_replace("\n\r", "\n", mnet_get_public_key($mnet_peer->wwwroot, $mnet_peer->application));
        $mnet_peer->public_key = preg_replace('/\n$/s', '', $mnet_peer->public_key);
        // PATCH-.
