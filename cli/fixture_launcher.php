 ...
 * 
 * Each worker will launch one time the fixture script that must reside in the current local/vmoodle/cli
...
foreach ($joblist as $jl) {
    ...
    if (!empty($jl)) {
        ...
        $hids = implode(',', $jl);
        $workercmd = "$phpcmd \"{$CFG->dirroot}/local/vmoodle/cli/fixture_worker.php\" --nodes=\"$hids\" $logattr --fixture={$options['fixture']} $verboseattr";
